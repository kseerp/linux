...
static const char *const adi_emu_test_modes[] = {
	[TEST_OFF] = "off",	      [TEST_MIDSCALE] = "midscale",
			    ...
			    int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		ret = regmap_write(st->regmap, ADI_EMU_REG_CNVST,
				   ADI_EMU_MASK_CNVST);
		if (ret)
			return ret;
...
static int adi_emu_reg_access(struct iio_dev *indio_dev, unsigned int reg,
			    ...
			    char *buf)
{
	...
	case SCRATCH_PAD:
		ret = regmap_read(st->regmap, ADI_EMU_REG_SCRATCH_PAD,
				  &reg_val);
		if (ret)
			return ret;
			     ...
			     const char *buf, size_t len)
{
	...
	case SCRATCH_PAD:
		...
		ret = regmap_write(st->regmap, ADI_EMU_REG_SCRATCH_PAD,
				   reg_val);
		if (ret)
			break;
		break;
...
static IIO_DEVICE_ATTR(scratch_pad, S_IRUGO | S_IWUSR, adi_emu_show,
...
static irqreturn_t adi_emu_trigger_handler(int irq, void *p)
{
	...
	if (*indio_dev->active_scan_mask & BIT(0)) {
		ret = regmap_read(st->regmap, ADI_EMU_REG_CH0_DATA_HIGH, &high);
		if (ret)
			return ret;

		ret = regmap_read(st->regmap, ADI_EMU_REG_CH0_DATA_LOW, &low);
		if (ret)
			return ret;
	...
	if (*indio_dev->active_scan_mask & BIT(1)) {
		ret = regmap_read(st->regmap, ADI_EMU_REG_CH1_DATA_HIGH, &high);
		if (ret)
			return ret;

		ret = regmap_read(st->regmap, ADI_EMU_REG_CH1_DATA_LOW, &low);
		if (ret)
			return ret;
...
static const unsigned long adi_emu_available_scan_masks[] = {
	BIT(0), BIT(1),
};
...
static const struct spi_device_id adi_emu_id[] = {
	{ "iio-adi-emu", 0 },
	{ }
};
MODULE_DEVICE_TABLE(spi, adi_emu_id);

static const struct of_device_id adi_emu_of_match[] = {
	{ .compatible = "iio-adi-emu", },
	{ }
};
MODULE_DEVICE_TABLE(of, adi_emu_of_match);
...
static struct spi_driver adi_emu_driver = {
	.driver = {
		.name = "iio-adi-emu",
		.of_match_table = adi_emu_of_match,
	},
	.probe = adi_emu_probe,
	.id_table = adi_emu_id,
};
